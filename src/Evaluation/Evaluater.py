...
from LLM.HallucinationDetector import LettuceDetect
from LLM.HallucinationDetector import HalluciNot
...
class Evaluater:
    ...
    def LLM_As_Judge(self, qids, questions, predicted_answers, true_answers, sample_size = 20):
        ...
        for i in tqdm(range(0, len(sample_indices))):

            question = [Q[i]]
            answer = [AP[i]]

            try:
                verdicts = self.judge.answer(question, answer)
